...
import { fetchTpoJobs } from '../redux/tpo/tpoJobs/tpoJobsSlice';
import { Row, Col, Spin, Result, Typography, Input, Button, Select, Divider } from 'antd';
import TpoJobCard from '../components/TpoJobCard';
...
const AllTpoJobs = () => {
  ...
  const { jobs, loading, error } = useSelector((state) => state.tpoJobs);
  const [search, setSearch] = useState('');
  const [location, setLocation] = useState('');
  const [jobType, setJobType] = useState('');

  useEffect(() => {
    dispatch(fetchTpoJobs());
  }, [dispatch]);
